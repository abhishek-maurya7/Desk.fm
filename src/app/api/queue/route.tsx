...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[POST /queue] Session result:", { userId: session?.user?.id ?? null });
    ...
    let existingTrack = await db
      .collection("tracks")
      .findOne({ trackId });
    console.log("[POST /queue] Existing track lookup result:", existingTrack ? { _id: existingTrack._id.toString() } : "not found");

    if (!existingTrack) {
      console.log("[POST /queue] Track not found — fetching YouTube metadata for trackId:", trackId);
      const metadata = await getYouTubeMetadata(trackId);
      console.log("[POST /queue] YouTube metadata result:", metadata);

      if (!metadata) {
        console.error("[POST /queue] Failed to fetch YouTube metadata for trackId:", trackId);
        return NextResponse.json(
      ...
      const insertTrackResult = await db
        .collection("tracks")
        .insertOne(metadata);
      console.log("[POST /queue] Track inserted with ID:", insertTrackResult.insertedId.toString());
    ...
    console.log("[POST /queue] Last queue entry:", lastQueueEntry ? { _id: lastQueueEntry._id.toString(), position: lastQueueEntry.position } : "none");

    const nextQueuePosition =
      typeof lastQueueEntry?.position === "number"
        ? lastQueueEntry.position + 1
        : 1;
    console.log("[POST /queue] Computed next queue position:", nextQueuePosition);
    ...
    console.log("[POST /queue] Queue item inserted with ID:", insertQueueResult.insertedId.toString());
    ...
    console.log("[POST /queue] Assembled queue item payload:", queueItemPayload);

    console.log("[POST /queue] Initializing PartySocket — host:", process.env.NEXT_PUBLIC_PARTYKIT_HOST, "room:", roomId);
    ...
    partySocket.addEventListener("open", () => {
      console.log("[POST /queue] PartySocket opened — sending TRACK_ADDED event");
      partySocket.send(
        JSON.stringify({
          type: "TRACK_ADDED",
...
export async function PATCH(req: NextRequest) {
  ...
  const session = await auth();
  console.log("[PATCH /queue] Session result:", { userId: session?.user?.id ?? null });
  ...
  console.log("[PATCH /queue] Parsed request body:", { queueId, roomId, status });
  ...
  console.log("[PATCH /queue] Updating queue item:", { queueId, roomId, newStatus: status });
  ...
  if (result.matchedCount === 0) {
    console.warn("[PATCH /queue] No matching queue item found for:", { queueId, roomId });
    return NextResponse.json(
